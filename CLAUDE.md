...
- *2026-08-12 — arm joint effort raised from a placeholder `1` to `4.1` Nm (`servo_effort`, XM430-W350 stall torque at 12 V) in [body.xacro](rosbot_description/urdf/open_manipulator/body.xacro) — **affects HW too**; at `1` Nm joint2 could not hold the arm out and settled ~0.2 rad short of any reach-down command, landing the gripper ~30 mm high. Gripper prismatic effort left at `1` (15 N tried, did not help closing against contact, reverted). Two **sim-only** additions gated on `use_sim` via new `wrist_roll`/`taper_jaws` xacro args: (1) `joint5` wrist roll between `link5` and the gripper, on its own `wrist_roll_controller` and deliberately outside the MoveIt `manipulator` group/SRDF (rolling about the tool axis does not move the grasp point, so joints 1-4 kinematics are untouched); its `<ros2_control>` is a separate `WristRollSystem` block rather than a fork of the upstream `open_manipulator_description` macro. (2) tapered V jaws replacing the stock flat palms, pads pitched 18° with a 120° included V, **box collisions on purpose** — a mesh collision gets convex-hulled and the V notch disappears. HW path renders neither (`dynamixel_hardware_interface` is pinned to `number_of_joints=5`, cf. 2026-05-19). Guards: `test_wrist_roll_and_taper_jaws_are_simulation_only`, `test_taper_jaw_collisions_are_convex_primitives`. **The V-pad geometry was superseded on 2026-08-20 — see below; only the `taper_jaws` arg name survives it.***
- *2026-08-20 — same branch, follow-on: the V pads are replaced by a user-CAD claw ([claw/Gripper-{Left,Right}.stl](rosbot_description/meshes/rosbot_xl/claw/) visual + a **generated** 32/25-box collision proxy in [claw_collision.xacro](rosbot_description/urdf/open_manipulator/claw_collision.xacro), regenerated by `grab_sequence/scripts/mesh_to_collision.py` in a sibling repo — do not hand-edit), and `gripper_lower` drops to `-0.023` under `taper_jaws` so the 26 mm gap the shuttlecock cork slipped through closes (84.0 mm open / 0.0 mm closed measured; MoveIt's SRDF `Close` state is still `-0.009`, so only direct JTC commands reach the new stop). **Two changes here are NOT sim-gated and affect HW:** the arm mount moves `-0.11 → +0.065` on `cover_link` ([rosbot_xl.urdf.xacro](rosbot_description/urdf/rosbot_xl.urdf.xacro)) and `LDR06` moves to `-0.125 0.0 0.07` in [manipulation.yaml](rosbot_description/config/rosbot_xl/manipulation.yaml)/[manipulation_pro.yaml](rosbot_description/config/rosbot_xl/manipulation_pro.yaml) — HW needs the arm re-bolted forward and a 70 mm lidar riser to match. **Open items:** the MoveIt collision matrix was not regenerated after the mount move (SRDF still says `link1 ↔ rplidar_link "Adjacent"`), a measured ±5° forward blind sector remains from the arm's parked upper links, and the now-unused `taper_pad`/`taper_spine` macros are still in [body.xacro](rosbot_description/urdf/open_manipulator/body.xacro). Guards: `test_taper_jaws_open_wider_than_the_target`, `test_claw_closes_past_the_cork`, `test_lidar_sits_behind_the_arm_mount`.*
...
- *2026-08-20 — MoveIt `Close` named state tied to the rendered gripper stop. `rosbot_xl.srdf` takes a `taper_jaws` xacro arg (MoveItConfigsBuilder already loads it via `load_xacro()`, so no rename); default stays the hardware-safe `-0.009` while the sim claw gets `-0.023`. A single value could not serve both — `-0.023` is out of bounds on hardware and `setJointValueTarget` rejects it, `-0.009` left the sim jaws 28 mm apart around a 26 mm cork. `move_group.launch.py` gained `use_sim` and overrides the semantic description as it already does the URDF; `manipulator.yaml` passes it. Guard: `test_srdf_close_state_reaches_the_gripper_stop`. `servo.launch.py`/`dock.launch.py` still build unmapped configs.*
