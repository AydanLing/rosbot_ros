...
- XL configurations in [config/rosbot_xl/](rosbot_description/config/rosbot_xl/): `basic`, `telepresence`, `autonomy` (LDR06+CAM11), `manipulation`, `manipulation_pro`, `custom`. Component schema from `husarion_components_description`. The two `manipulation*` files place `LDR06` at `-0.125 0.0 0.07` — behind and above the arm mount at `+0.065`, which the arm's swept volume and base column both constrain. **The two positions are coupled**: moving the arm invalidates the lidar pose (and vice versa). Both apply to HW as well as sim — see [MANIPULATOR.md](MANIPULATOR.md).
- `open_manipulator/` — [body.xacro](rosbot_description/urdf/open_manipulator/body.xacro) (links/joints, `servo_effort`, optional `joint5` wrist roll and CAD claw), [open_manipulator_x_macro.xacro](rosbot_description/urdf/open_manipulator/open_manipulator_x_macro.xacro) (assembles body + upstream `OpenManipulatorXSystem` + the sim-only `WristRollSystem` `<ros2_control>` block). Takes `wrist_roll` / `taper_jaws` xacro args, which `rosbot_xl.urdf.xacro` ties to `use_sim`.
- **Generated file, cross-repo:** [open_manipulator/claw_collision.xacro](rosbot_description/urdf/open_manipulator/claw_collision.xacro) holds the CAD claw's box collision proxy (32 left / 25 right boxes voxelised at 8 mm from `meshes/rosbot_xl/claw/Gripper-{Left,Right}.stl`). Do not hand-edit — it is produced by `grab_sequence/scripts/mesh_to_collision.py`, which lives in a sibling repo. Boxes rather than a mesh collision because the physics engine convex-hulls mesh collisions, filling in the concave grip pocket. Guarded by `test_taper_jaw_collisions_are_convex_primitives`.
